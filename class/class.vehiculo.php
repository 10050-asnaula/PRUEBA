...
class vehiculo{
	...
	if($bandera){
		...
		$html = '
		<form name="Form_vehiculo" method="POST" action="PrincipalV.php" enctype="multipart/form-data">
		...
		<input class="form-control" type="hidden" name="op" value="' . $op  . '">
			<table class="table table-striped table-dark" border="2" align="center">
				...
				<tr>
					<td>Placa:</td>
					<td><input class="form-control" type="text" size="6" name="placa" value="' . $this->placa . '"></td>
				</tr>
				<tr>
					<td>Marca:</td>
				...
				<tr>
					<td>Motor:</td>
					<td><input class="form-control" type="text" size="15" name="motor" value="' . $this->motor . '"></td>
				</tr>	
				<tr>
					<td>Chasis:</td>
					<td><input class="form-control" type="text" size="15" name="chasis" value="' . $this->chasis . '"></td>
				</tr>
				<tr>
					<td>Combustible:</td>
				...
				<tr>
					<td>Avalúo:</td>
					<td><input class="form-control" type="text" size="8" name="avaluo" value="' . $this->avaluo . '" ' . $flag . '></td>
				</tr>
				<tr>
					<th colspan="2"><input class="btn btn-outline-info" type="submit" name="Guardar" value="Guardar">&nbsp&nbsp&nbsp<a class="btn btn-outline-warning" href="PrincipalV.php" role="button">Regresar</a></tr>
	...
	public function get_list(){
		...
		<table class="table table-striped table-dark" border="2" align="center">
			...
			<tr>
				<th colspan="8"><a class="btn btn-outline-success" href="PrincipalV.php?d=' . $d_new_final . '" role="button">Nuevo</a></th>
			</tr>
			<tr>
				<th>Placa</th>
				<th>Marca</th>
			...
			*/
				...
				$html .= '
					<tr>
						...
						<td>' . $row['anio'] . '</td>
						<td>' . $row['avaluo'] . '</td>
						<td><a class="btn btn-outline-secondary" role="button" href="PrincipalV.php?d=' . $d_del_final . '">Borrar</a></td>
						<td><a class="btn btn-outline-secondary" role="button" href="PrincipalV.php?d=' . $d_act_final . '">Actualizar</a></td>
						<td><a class="btn btn-outline-secondary" role="button" href="PrincipalV.php?d=' . $d_det_final . '">Detalle</a></td>
					</tr>';
...
//********************************************************************************************************
	...
	private function _get_radio($arreglo,$nombre,$defecto=NULL){
		$html = '
		<table class="table-dark" border=0 align="left">';
		foreach($arreglo as $etiqueta){
			$html .= '
			<tr>
				<td>' . $etiqueta . '</td>
				<td>';
				$html .= ($defecto == $etiqueta)? '<input class="form-control" type="radio" value="' . $etiqueta . '" name="' . $nombre . '" checked/></td>':'<input class="form-control" type="radio" value="' . $etiqueta . '" name="' . $nombre . '"/></td>';
...
public function get_detail_vehiculo($id){
	...
	if($num == 0){
		...
		<table class="table table-striped table-dark" border="2">
			...
			<tr>
				<th colspan="2"><a class="btn btn-outline-warning" href="PrincipalV.php" role="button">Regresar</a></th>
			</tr>																						
		</table>';
...
//***************************************************************************************************************************
	...
	private function _message_error($tipo){
		...
		<table border="0" align="center">
			...
			<tr>
				<th><a class="btn btn-outline-warning" href="PrincipalV.php" role="button">Regresar</a></th>
			</tr>
		</table>';
		return $html;
	}
